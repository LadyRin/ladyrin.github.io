...
const emit = defineEmits(['closeApp', 'maximize', 'minimize', 'openApp'])
...
<template>
  <div class="window-manager">
    <WindowComponent
      v-for="window in apps"
      :app="window"
      :z-index="zIndices.get(window.pid)"
      :key="window.pid"
      @place-on-top="placeOnTop(window.pid)"
      :appear-at="nextWindowPosition()"
      @close="$emit('closeApp', window.pid)"
      @maximize="maximize"
      @minimize="minimize">
      <DolphinComponent
        v-if="window.name == 'Dolphin File Manager'"
        :path="window.args.path"
        @open-app="openApp" />
      <FirefoxComponent
        v-if="window.name == 'Firefox'"
        :path="window.args.path"
        @open-app="openApp" />
    </WindowComponent>
  </div>
