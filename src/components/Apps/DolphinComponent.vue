...
const fscomponents = ref<FileSystemComponent[]>(
  (fs.getWorkingDirectory() as FSDirectory).getChildren()
)
  ...
  <div class="dolphin-file-manager">
    <div class="header">
      ...
      <div>
        <div class="path">
          <span
            v-for="(part, index) in pathToDisplay"
            :key="index"
            @click="cd(pathToDisplay.slice(0, index + 1).join('/'))">
            {{ part }}&nbsp;&gt;
          </span>
          <span>
    ...
    <div class="content" :class="{ empty: icons.length == 0 }">
      <p v-if="icons.length == 0">This folder is empty</p>
      <DesktopAppIcon
        class="icon"
        v-else
        v-for="icon in icons"
