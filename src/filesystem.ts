...
export class FileSystem {
  ...
  private constructor() {
    ...
    projets.addChild(
      new FSFile('E:cclesia.html', 'content/projects/ecclesia.html', 'text-html.svg')
    )
    projets.addChild(
      new FSFile('Exactly Enough Items.html', 'content/projects/eei.html', 'text-html.svg')
    )
...
export class FileSystemExplorer {
  ...
  ls(): string[] {
    return (this.workingDirectory as FSDirectory)
      .getChildren()
      .map((child) => child.getName() + (child.getType() === 'directory' ? '/' : ''))
  }
  ...
  exists(path: string): boolean {
    ...
    for (const pathComponent of pathComponents) {
      if (pathComponent === '..') {
        ...
        component =
          (component as FSDirectory)
            .getChildren()
            .find((child) => child.getName() === pathComponent) || component
      }
    }
